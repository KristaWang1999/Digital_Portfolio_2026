...
const App: React.FC = () => {
  ...
  // --------------------------------------------------------------------------
  // Comchat Detail Page - Full Screen Long Image
  // --------------------------------------------------------------------------
  const ComchatDetail = () => {
  ...
  // --------------------------------------------------------------------------
  // Afrotunes Detail Page - Full Screen Long Image
  // --------------------------------------------------------------------------
  const AfrotunesDetail = () => {
    // 占位符图片 - 您可以将此 URL 替换为本地长图的路径
    const longImagePlaceholder = "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?q=80&w=2000&auto=format&fit=crop";
    
    return (
      <div className="w-full bg-[#050505] min-h-screen animate-[fadeIn_0.8s_ease-out]">
        <img 
          src={longImagePlaceholder} 
          alt="Afrotunes Project Detail" 
          className="w-full h-auto block"
        />
      </div>
    );
  ...
  // --------------------------------------------------------------------------
  // Boomplay Detail Page - Full Screen Long Image
  // --------------------------------------------------------------------------
  ...
  return (
    <div className="min-h-screen selection:bg-[#7700FF] selection:text-white bg-[#050505]">
      ...
      <section id="home" className="relative h-screen w-full flex flex-col items-center justify-center px-6 overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-[1200px] mx-auto flex flex-col items-center text-center translate-y-4">
          <h1 className="flex flex-col items-center mb-16 opacity-0 animate-[fadeIn_1s_ease-out_0.3s_forwards] w-full overflow-visible">
            <span 
              className={`text-[7.2vw] md:text-[6.1vw] font-bold leading-none tracking-[0.11em] transition-all duration-700 cursor-default inline-block ${isPortfolioHovered ? 'text-[#7700FF]' : 'text-white'}`}
              onMouseEnter={() => setIsPortfolioHovered(true)}
              onMouseLeave={() => setIsPortfolioHovered(false)}
            >
              {t.hero.titlePrefix}
            </span>
            <span className="text-[6.4vw] md:text-[5.4vw] font-bold text-white/90 leading-none whitespace-nowrap mt-3 md:mt-4 tracking-[0.04em] uppercase">
              {t.hero.titleSuffix}
            </span>
          </h1>

          <div className="max-w-[800px] mx-auto opacity-0 animate-[fadeIn_1s_ease-out_0.6s_forwards]">
            <p className="text-gray-400 text-base md:text-lg font-normal leading-relaxed mb-14 px-4">
              {t.hero.subtitle}
            </p>
                ...
                <div className="space-y-8">
                  {EDUCATION.map(edu => (
                    <div key={edu.id} className="relative group">
                      ...
                      <h5 className="text-[16px] font-bold text-white mb-0.5 leading-tight">{edu.school} - {edu.degree}</h5>
                    </div>
                  ))}
                </div>
              </div>
                      ...
                      >
                         ...
                         <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 relative z-10">
                           <div className="flex flex-col gap-2">
                             <h4 className="text-[18px] font-bold tracking-tight text-white/90 group-hover:text-white transition-colors">{exp.company}</h4>
                         ...
                         <div className="mt-8 pt-8 border-t border-white/5 relative z-10">
                           <ul className="space-y-5">
                             {exp.description.map((line, i) => (
                               <li key={i} className="text-base text-gray-500 flex items-start gap-5 leading-relaxed group-hover:text-gray-300 transition-all duration-500">
                                 ...
                                 <span className="tracking-wide font-normal">{line}</span>
                               </li>
                             ))}
                           </ul>
                         </div>
      ...
      {/* Contact Section */}
      <section id="contact" className="pt-16 pb-[94px] px-6 flex flex-col items-center justify-center text-center bg-[#050505] relative overflow-hidden">
        <div className="max-w-[1200px] mx-auto w-full relative">
          ...
          <div className="opacity-0 animate-[fadeIn_1s_ease-out_0.3s_forwards] z-10 flex flex-col items-center">
            <a 
      ...
      {/* Footer Section */}
      <footer className="pt-12 pb-20 border-t border-white/5 bg-[#050505]">
        <div className="max-w-[1200px] mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-10">
          <div className="flex items-center">
            <p className="text-base text-white/40 font-medium tracking-tight">
              © Copyright 2026
            </p>
          </div>
      ...
      {selectedProject && (
        <div className="fixed inset-0 z-[200] overflow-y-auto bg-[#050505] animate-[fadeIn_0.5s_cubic-bezier(0.19,1,0.22,1)]">
          ...
          {selectedProject.id === 'boomplay' ? (
            <BoomplayDetail />
          ) : selectedProject.id === 'afrotunes' ? (
            <AfrotunesDetail />
          ) : selectedProject.id === 'comchat' ? (
            <ComchatDetail />
          ) : (
              ...
              <div className="mb-32">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-20 py-16 border-y border-white/5">
                  <div>
                    ...
                    <p className="text-gray-400 leading-relaxed text-base md:text-lg">{selectedProject.challenge}</p>
                  </div>
                  <div>
                    <h4 className="text-white/30 text-[10px] mono uppercase mb-6 tracking-widest uppercase">{t.modal.solution}</h4>
                    <p className="text-gray-400 leading-relaxed text-base md:text-lg">{selectedProject.solution}</p>
                  </div>
                  <div>
          ...
          <div className="py-20 text-center">
             <button 
              onClick={() => setSelectedProject(null)}
              className="px-12 py-5 border border-white/10 rounded-full hover:bg-white hover:text-black transition-all mono uppercase tracking-widest text-xs font-bold"
             >
               Close Case Study
             </button>
          </div>
